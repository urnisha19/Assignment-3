...
console.log(budgetCalculator(2, 1, 1)); //calling the function budgetCalculator() and gave three parameter 2,1,1 as the numbers of watch,phone and laptop respectively to calculate and show the total budget.
...
function megaFriend(friends) {
    ...
    for (var i = 0; i < friends.length; i++) {
        if (friends[i].length > highestLength) {
            var highestLength = friends[i].length;
            var longest = friends[i];
        }
    }
    return longest;
}
console.log(megaFriend(["Lima", "Eva", "Puja", "Susmita"])); //calling the function  megaFriend() and gave an array as the parameter to find the longest string of the array
